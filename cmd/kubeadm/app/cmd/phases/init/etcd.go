...
var (
	etcdLocalExample = cmdutil.Examples(`
		# Generates the static Pod manifest or service unit file for etcd, functionally
...
func NewEtcdPhase() workflow.Phase {
	phase := workflow.Phase{
		Name:  "etcd",
		Short: "Generate static Pod manifest or service unit file for local etcd",
...
func newEtcdLocalSubPhase() workflow.Phase {
	phase := workflow.Phase{
		Name:         "local",
		Short:        "Generate the static Pod manifest or service unit file for a local, single-node local etcd instance",
	...
	return func(c workflow.RunData) error {
		...
		if cfg.Etcd.External == nil {
			...
			if data.ServiceHosting() {
				fmt.Printf("[etcd] Creating service unit file for local etcd in %q\n", data.ServiceUnitDir())
				if err := etcdphase.CreateServiceUnitFile(data.ServiceUnitDir(), cfg.NodeRegistration.Name, &cfg.ClusterConfiguration, &cfg.LocalAPIEndpoint); err != nil {
					return errors.Wrap(err, "error creating local etcd service unit file")
				}

				if data.DryRun() {
					fmt.Print("[dryrun] Would run system service for etcd\n")
				} else {
					return etcdphase.RunService()
				}
			} else {
			...
			klog.V(1).Infoln("[etcd] External etcd mode. Skipping the local etcd initialization")
		}
